...
def download_kaggle_dataset(output_dir="./data"):
    ...
    try:
        ...
        if (
            not kaggle_token
            and not kaggle_username
            and not kaggle_key
            and not kaggle_json_path.exists()
        ):
            raise ValueError(
        ...
        api.dataset_download_files(
            dataset=dataset_name, path=str(output_path), unzip=True
        )
        ...
        if env_path.exists():
            ...
            print(
                f"  KAGGLE_API_TOKEN: {'Set' if os.getenv('KAGGLE_API_TOKEN') else 'Not set'}"
            )
            print(
                f"  KAGGLE_USERNAME: {'Set' if os.getenv('KAGGLE_USERNAME') else 'Not set'}"
            )
        ...
        print(
            "   - Or add to .env file: KAGGLE_USERNAME=your_username and KAGGLE_KEY=your_key"
        )
        raise
